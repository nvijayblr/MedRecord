'use strict';
medVision.controller('acqCtrl', ['$scope', 'BaseServices', '$timeout', function(_scope, _services, _timeout){
	...
	_scope.init = function(_ehrId) {
		...
		_services.http.serve({
			method: 'GET',
			url: 'https://dev.medrecord.nl/ehr/'+_ehrId+'/procedure/acq/omh?authToken=helloletmeinplease'
		}, function(data){
			_scope.bm = {key: "Body Mass", bar: true, values: []};
			angular.forEach(data.data, function(_obj, _i){
				_scope.dt = moment(_obj.body.effective_time_frame.date_time).format('X');
				_scope.bm.values.push({"x": _scope.dt, "y": _obj.body.score});
			});
			_scope.bm = [_scope.bm];
